...
class ChainOfThoughtNode(Node):
    ...
    def exec(self, prep_res):
        ...
        print(f"{'═' * 62}")
        print(f"  ▶ THOUGHT {current_thought_number}  —  LLM INPUT")
        print(f"{'═' * 62}")
        print(prompt)
        print(f"\n{'─' * 62}")
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        # Push thought to llm-canvas if a canvas session is active in shared
        if "_canvas" in shared:
            try:
                canvas = shared["_canvas"]
                thought_num = exec_res.get("thought_number", "?")
                prompt_summary = (
                    f"Thought {thought_num} — Problem: {prep_res['problem']}\n\n"
                    f"Plan going in:\n{format_plan(exec_res.get('planning', []), indent_level=1)}"
                )
                user_node = canvas.add_message(
                    {"content": prompt_summary, "role": "user"},
                    parent_node_id=shared.get("_canvas_parent_id"),
                )
                asst_node = canvas.add_message(
                    {"content": dedented_thinking, "role": "assistant"},
                    parent_node_id=user_node["id"],
                )
                shared["_canvas_parent_id"] = asst_node["id"]
            except Exception as _canvas_err:
                print(f"  [llm-canvas] error: {_canvas_err}")
        ...
        print("\n  ▶ SHARED STATE")
        print(f"{'─' * 62}")
        print(f"  current_thought_number : {shared.get('current_thought_number')}")
        print(
            f"  thoughts               : {len(shared.get('thoughts', []))} thought(s) stored"
        )
        ...
        print(f"  next_thought_needed    : {exec_res.get('next_thought_needed', True)}")
        print(f"{'─' * 62}")
